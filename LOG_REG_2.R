...
modelFit<- train(DEATH_EVENT~ejection_fraction+serum_creatinine,
                 method='glm',
...
testing$predict <- predict(modelFit, newdata = testing[,-3])
for(i in 1:90)
{
  if (testing[i,4]>=0.5)
    testing[i,4]=1
...
roc_obj<-roc(testing[,3],as.numeric(testing$predict))
rocauc<-auc(roc_obj)

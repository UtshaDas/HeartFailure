...
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/Heart_Failure/Sap_DT_ALL.csv")
...
for (i in 1:20) {
  for (j in 1:32) {
    ...
    testing$pred <- predict(object = prepruned_model,  newdata = testing[,-12],    type = "class")
    #printcp()
    #plotcp()
    ...
    roc_obj<-roc(testing[,12],as.numeric(testing$pred))
    rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/Heart_Failure/Sap_DT_ALL.csv")
print(cp_op)
print(max_dep)
print(min_split)
print(accu)

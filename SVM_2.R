...
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/Heart_Failure/Sap_SVM_Selected.csv")
...
for(i in 1:length(seedarray)){
  ...
  system.time(svm_tune <- tune(svm, train.x=training[,1:2], train.y=as.factor(training[,3]), 
  ...
  print(classifier)
  y_pred=predict(classifier,newdata=testing[-3])
  #y_pred<-round(y_pred)
  cm=table(y_pred,testing[,3],dnn=c("Prediction","Actual"))
  ...
  roc_obj<-roc(testing[,3],as.numeric(y_pred))
  rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/Heart_Failure/Sap_SVM_Selected.csv")
